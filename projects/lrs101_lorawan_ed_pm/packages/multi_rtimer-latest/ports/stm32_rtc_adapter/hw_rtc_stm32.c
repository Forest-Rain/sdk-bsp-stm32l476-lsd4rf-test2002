...
#define CONV_DENOM                                  ( 1 << ( N_PREDIV_S - COMMON_FACTOR ) )    // 2^(10 - 3) / ( 1000 / 2^3) = 1024/1000 
 ...
 * Set with the \ref rtc_set_timer_context function
 ...
 */
uint32_t rtc_ms2tick( uint32_t milliseconds )
{
...
void RtcDelayMs( uint32_t delay )
{
    ...
    uint64_t refTicks = rtc_get_timer_value( );

    delayTicks = rtc_ms2tick( delay );

    // Wait delay ms
    while( ( ( rtc_get_timer_value( ) - refTicks ) ) < delayTicks )
    {
        __NOP( );
    }
}
...
uint32_t rtc_get_calendar_time( uint32_t *milliseconds )
{
    ...
    *milliseconds = rtc_tick2ms( ticks );
