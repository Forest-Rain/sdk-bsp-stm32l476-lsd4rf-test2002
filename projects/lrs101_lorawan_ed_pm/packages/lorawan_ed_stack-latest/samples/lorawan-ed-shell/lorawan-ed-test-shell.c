...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#ifndef LORAWAN_ED_STACK_DEBUG_SHELL_TEST
#define LORAWAN_ED_STACK_DEBUG_SHELL_TEST                0
#endif
...
#ifndef LORAWAN_ED_STACK_DEBUG_SHELL_TEST_APP_DATA
#define LORAWAN_ED_STACK_DEBUG_SHELL_TEST_APP_DATA       0
#endif
/* Private macro -------------------------------------------------------------*/
...
static uint16_t max_tx_nbtrials = 20; // for join and tx data
static uint32_t rx_correct_cnt = 0;
static uint8_t auto_periodic_tx_enable = true;
...
lorawan_ed_params_t lorawan_ed_init_params =
{
    ...
    (DeviceClass_t)LORAWAN_ED_STACK_MAC_PARAMETER_DEVICE_TYPE,
    ...
    while (1)
    {
                                            ...
                                            RT_WAITING_FOREVER, &ev) == RT_EOK)
        {
            ...
            if (ev == EV_LORAWAN_APL_START_JOIN_NETWORK)
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO,
            ...
            else if (ev == EV_LORAWAN_APL_PERIODIC_TX_DATA)
            {
                ...
                if (ping_test_flag)
                {
                    ...
                    if (!tx_seq_cnt)
                    {
                        LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Pinging with %d bytes of data for %d counters:", payload_len, max_tx_nbtrials);
                    }
...
static void lorawan_ed_joined(void)
{
    ...
    // go to send packet periodicly
    if( auto_periodic_tx_enable )
    {
        // periodic tx,clear this parameter
        max_tx_nbtrials = 0;
...
{
#if LORAWAN_ED_STACK_DEBUG_SHELL_TEST_APP_DATA == 1
    if(app_data->BuffSize)
    {
        LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_APP_DATA, LOG_LVL_INFO, "[App Payload(%d)]:",app_data->BuffSize);
        for( uint8_t i = 0; i < app_data->BuffSize; i++ )
        {
            LORAWAN_ED_DEBUG_LOG_RAW(LORAWAN_ED_STACK_DEBUG_SHELL_TEST_APP_DATA, " %02X",app_data->Buff[i]);
        }
        LORAWAN_ED_DEBUG_LOG_RAW(LORAWAN_ED_STACK_DEBUG_SHELL_TEST_APP_DATA, "\r\n");
    }
#endif
    ...
    switch (app_data->Port)
    {
        ...
        case LORAWAN_ED_STACK_MAC_PARAMETER_APPLICATION_FPORT:
        {
            if ( app_data->BuffSize == 1 )
            {
                ...
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_DBG, "LED OFF", app_data->Port);
                    LED_Off(LED_BLUE) ;
                }
                else
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_DBG, "LED ON", app_data->Port);
                    LED_On(LED_BLUE) ;
                }
            }
            break;
        ...
        default:
            ...
            if( ping_test_flag )
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Reply from Server:seqno=%d, dn_fcnt=%d, bytes=%d, Total time=%d ms, ed_rssi=%d, ed_snr=%d, gw_demod_margin=%d, gw_received_nb=%d",
...
{ 
    if( ping_test_flag )
    {
        ...
        {
            LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Request Server timed out: seqno=%d, time=%d ms", tx_seq_cnt, ( rx_timestamp - tx_timestamp ) );
        }
    }
}
    ...
    { 
        ...
        else
        {
            if( ping_test_flag )
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "-> Tx pakcets: sent = %d, tx_total = %d.%d KByte",tx_seq_cnt, tx_total_kbyte_integer, tx_total_kbyte_decimal);
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "-> Rx pakcets: received = %d, lost = %d, per = %d%, rx_total = %d.%d KByte",rx_correct_cnt, tx_seq_cnt - rx_correct_cnt, per,rx_total_kbyte_integer,rx_total_kbyte_decimal);
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "--> End-Device Rx rssi: max_rssi = %d, min_rssi = %d, avg_rssi = %d",rssi_value_max,rssi_value_min,avg_ed_rssi);
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "--> End-Device Rx snr : max_snr  = %d, min_snr  = %d, avg_snr  = %d",snr_value_max,snr_value_min,avg_ed_snr);
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "--> Gateway RX DemodMargin : max_margin = %d, min_margin = %d, avg_margin = %d",gw_demod_margin_value_max,gw_demod_margin_value_min,avg_gw_demod_margin);
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "--> Gateway Numbers : %d",gw_received_nb);
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "====== LoRaWAN End-Device Ping Test Finished ======\r\n");
            }
        }
    }        
}
...
static void lorawan_ed_confirm_device_class(DeviceClass_t Class)
{
    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Switch to Class %c Done", "ABC"[Class]);
    /*Optionnal*/
...
int lorawan_test_shell_init(void)
{
    if( lorwan_ed_test_shell_thread.entry == RT_NULL)
    {
        ...
        {
            LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "lorwan_ed_test_shell_thread creat fail!");
        }
    }
    return 0;
}
...
static void lorawan_shell_usage(void)
{
    size_t i = 0;
    /* parameter error */
    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Usage:");
    for (i = 0; i < sizeof(lorawan_help_info) / sizeof(char*); i++) 
    {
        LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "%s", lorawan_help_info[i]);
    }
    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "\r\n");
}
       ...
       /* SET \ GET */
        ...
        if (!rt_strcmp("deveui", cmd))
        {
            if( argc > 2 )
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "DevEUI=%02X%02X%02X%02X%02X%02X%02X%02X", HEX8(deveui));

                if( lorawan_ed_set_deveui(deveui) == RT_EOK )
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, " Set OK");
                }
                else
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, " Set Failed");
                }
            }
            else
            {
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "DevEUI=%02X%02X%02X%02X%02X%02X%02X%02X", HEX8(lorawan_ed_get_deveui()));
            }
        }
        else if (!rt_strcmp("joineui", cmd))
        {
            ...
            if( argc > 2 )
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "JoinEUI=%02X%02X%02X%02X%02X%02X%02X%02X", HEX8(joineui));

                if( lorawan_ed_set_joineui(joineui) == RT_EOK )
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, " Set OK", HEX8(joineui));
                }
                else
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, " Set Failed", HEX8(joineui));
                }
            }
            else
            {
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "JoinEUI=%02X%02X%02X%02X%02X%02X%02X%02X", HEX8(lorawan_ed_get_joineui()));
            }
        }
        else if (!rt_strcmp("appkey", cmd))
        {
            if( argc > 2 )
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "AppKey=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X", HEX16(appkey));
            ...
            {
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "AppKey=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X", HEX16(lorawan_ed_get_appkey()));
            }
        }
        else if (!rt_strcmp("devaddr", cmd))
        {
            if( argc > 2 )
            {
               ...
               LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "DevAddr=%08X", devaddr[0] << 24 | devaddr[1] << 16 | devaddr[2] << 8 | devaddr[3]);
               ...
               {
                   LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, " Set Fail");
               }
#else
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Error,Enable ABP function Please.");
#endif
            }
            else
            {
                uint32_t devaddr = lorawan_ed_get_devaddr();
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "DevAddr=%08X", devaddr);
            }
        }
        else if (!rt_strcmp("appskey", cmd))
        {
            if (argc > 2)
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "AppSKey=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",HEX16(appskey));
            ...
            {
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "AppSKey=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
        ...
        else if (!rt_strcmp("nwkskey", cmd))
        {
            if (argc > 2)
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "AppSKey=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",HEX16(nwkskey));
        ...
        else if (!rt_strcmp("class", cmd))
        {
            ...
            if (argc > 2)
            {
                ...
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Class Type:%d Is NOT Supported", class_type);
                }
            }
            else
            {
            ...
            {
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Class Type(Setup,Current): %c, %c", "ABC"[lorawan_ed_init_params.Class],"ABC"[(mibReq.Param.Class & 0x03)]);
            }
            else
            {
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Class Type: %c", "ABC"[lorawan_ed_init_params.Class]);
            }
        }
        else if (!rt_strcmp("nbtrials", cmd))
        {
            ...
            if( argc > 2 )
            {
                ...
                {
                    LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Nbtrials:%d Is NOT Supported",nbtrials);
                }
            }
            else
            {
            ...
            LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "nbtrials: %d", mibReq.Param.ChannelsNbTrans);
        }
        else if (!rt_strcmp("confirm", cmd))
        {
            ...
            LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Data Message Type: %s", lorawan_ed_init_params.DataMessageType?"Confirm":"UnConfirm");
        }
        else if (!rt_strcmp("activation", cmd))
        {
            ...
            LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Activation Type: %s", lorawan_ed_init_params.ActivationType?"ABP":"OTAA");
        }
        else if (!rt_strcmp("adr", cmd))
        {
            ...
            LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "ADR: %s", lorawan_ed_init_params.AdrEnable?"Enable":"Disable");
        }
        /* JOIN */
       else if (!rt_strcmp(cmd, "join"))
        {
            ...
            if (argc > 2)
            {
                ...
                if (argc > 3)
                {
                    ...
                    if (period < 8)
                    {
                        LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Join Period Error!");
                    }
                    else
                    {
                    ...
                    if (argc > 4)
                    {
                        /* auto periodic tx after joined */
                        auto_periodic_tx_enable = atoi(argv[3]);
                    }
                }
            }
            ...
            else
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Stop Join");
            }
        }
        else if (!rt_strcmp(cmd, "ping")) /* Ping test */
        {
            ...
            if ( lorawan_ed_join_status() == LORAWAN_ED_NOT_JOIN_NETWORK )
            {
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "==== Please Join Network first ====");
        ...
        else if (!rt_strcmp(cmd, "tx")) /* TX test */
        {
            ...
            else
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Stop Periodic Tx");
            }
        }
#ifdef PKG_USING_EASYFLASH
        else if(!rt_strcmp(cmd, "save")) /* save to flash */
        {
            ...
            if (!rt_strcmp(argv[2], "dev"))
            {
                ...
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "save dev done");
            }
            else // if (!rt_strcmp(argv[2], "cfg"))
            {
                lorawan_ed_save_cfg();
                LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "save cfg done");
            }
        }
        ...
        {
            LORAWAN_ED_DEBUG_LOG(LORAWAN_ED_STACK_DEBUG_SHELL_TEST, LOG_LVL_INFO, "Parameter Error");
        }
    }
    return 1;
}
